...
Current bridge version: `v0.6.0`.

The bridge now exposes 68 modular tools and covers the full normal loop:
...
detect duplicate/dead-code candidates
verify everything
commit/push
```

Code intelligence now has three levels:

```txt
regex        -> quick fallback
TypeScript AST -> per-file symbols/imports/exports/references
semantic Program/TypeChecker -> cross-file symbols and references
```

Dependency graph resolution now supports:

```txt
relative string matching
TypeScript module resolution through tsconfig/baseUrl/paths/barrels
```
...
Status: implemented for file-writing. `bridge_verify_all` was delivered later in v0.4.6.
...
### v0.6.0: project safety and bounded recovery

Status: implemented.

```txt
allowed-root / denied-path enforcement
secret-aware Git inspection and commit preflight
project_profile and project_profile_save
workspace snapshots, diffs, listing and guarded rollback
persistent cache TTL/limits/pruning
68 generated tool schemas with explicit risk annotations
```

Remaining useful work should be evidence-driven: deeper call attribution, optional retention controls, and project-specific workflows discovered while using the bridge on real repositories.
...
The allowed-roots/denied-path policy is implemented in v0.6.0. It constrains explicit Bridge path/cwd operations, blocks sensitive filenames and canonical-path escapes, and is intentionally documented as blast-radius reduction rather than an operating-system sandbox.
