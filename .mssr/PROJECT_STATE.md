...
## Current release
Bridge `0.6.134` is prepared in source with vendored and installed `@mauroprime/mssr 0.2.65`; the live HTTP runtime is still `0.6.133` until the controlled adoption/restart completes. Release 0.6.134 keeps the single `bridgeNotices` transport but changes host delivery policy: current unresolved attention is scoped to the active trace/project/workflow where possible, unrelated project notices become count-only in ordinary responses, full global recovery remains in `bridge_notice_history`, MSSR lifecycle transitions supersede prior pending wrappers, `resolved` becomes history-only, successful/info lifecycle events no longer occupy attention, oversized notices cannot head-of-line block delivery, and persisted old queues are reconciled on startup. Portable MSSR semantics remain unchanged at 0.2.65.
