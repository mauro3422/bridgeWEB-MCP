...
## Current release
Bridge `0.6.131` is the verified source/dist release candidate with vendored and installed `@mauroprime/mssr 0.2.64`; the live HTTP runtime remains 0.6.130 until durable publication and controlled watchdog adoption complete. The 0.6.131 change is Bridge-only: Project Situation now compares retained historical receipt evidence only when the exact `canonicalOwner + ref` remains a current authoritative repository candidate. Historical inbox receipts are preserved unchanged; genuine stale PROJECT_MEMORY/PROJECT_STATE revision mismatches remain REVIEW.
...
Bridge `0.6.131` source verification includes focused `npm run check`, `npm run build`, Project Situation regression/readback, plus full isolated `npm run test:regressions` PASS. The real inbox projection reports `evidenceComplete=true`, removes only the false `canonical-baseline-missing`, and preserves the two expected stale PROJECT_MEMORY/PROJECT_STATE mismatches with `revalidate-context-evidence`. The integrated regression liveness run completed 64/64 persisted writes with six synthetic agents, p95 concurrent readiness 2.72 ms, max 48.44 ms, zero event-loop stalls and one passive WAL checkpoint. The prior 0.6.130 post-release 8/12-agent stress evidence remains valid for transport liveness and is not reinterpreted as a per-tool latency SLA.
