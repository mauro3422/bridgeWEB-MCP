...
## Current release
Bridge `0.6.126` is source/dist/live current on the canonical final vendored and installed `@mauroprime/mssr 0.2.61` artifact (688166 bytes, SHA-256 `946ba46fbcbb9f7faf712dd0421a0ccffdf85f3c611fa133bd55f3ee3c03ca8a`). Controlled HTTP-only restart ack `0cb2edc8-c2ad-493d-a0bf-347c983b8912` adopted PID `9364`, boot `24162ab9-4ea1-4456-84ec-42c4b93d931f`; the Secure MCP tunnel remained `live/ready`. Runtime catalog is 162 tools and the live HTTP contract reports 120000 ms keep-alive. The fresh post-restart tunnel baseline reports zero new tunnel-service 502 and zero local `POST /mcp` no-status deltas. MSSR 0.2.61 Git source remains published at `0c3d9c58c073aea746394387532496bdca474fe4`; no public npm-registry publication is claimed.
...
Bridge `0.6.126` full verification completed with `ok=true` / `failedRequired=0` in job `bridge_verify_1789589039468_2` in about 159 s. It passed doctor, check/build, live HTTP smoke, dual-era MCP, full regressions, routing latency, WAL maintenance, observability liveness, skill routing, tools-doc checks, watchdog/metrics status and tool-catalog sanity. The dedicated liveness gate persisted 64/64 writes across 123 readiness probes with p95 1.48 ms and max 46.36 ms. Valid live concurrency stress completed 16/16 simultaneous MCP calls with zero tool errors in about 333 ms, and `project_context_load(includeGuides=false)` used the no-guide fast path. After final 0.6.126 adoption, live readback reported PID `9364`, boot `24162ab9-4ea1-4456-84ec-42c4b93d931f`, keep-alive 120000 ms, event-loop max lag 13 ms, zero recorded stalls and zero HTTP client errors; the fresh tunnel baseline still showed zero new 502/no-status transport failures.
