...
## Current release
Bridge `0.6.139` is source/dist/live current with exact `@mauroprime/mssr 0.2.71` from the verified vendored artifact (`c95a28aa4d5aac92eeda85a64d2254373c8178684aa6ae6484547df538e50af5`). Release commit `4de167566e79c07a877d114cdf07ed5fae5e83a4` is published on `origin/main`; controlled HTTP restart `392bcb0c-4261-49b5-bc54-531a964b1f4b` is the historical adoption event, and post-adoption strict gate `bridge_verify_1789850569079_1` passed with `ok=true`, `code=0`, `failedRequired=0`, and `strictGit=true`. Runtime PID/boot are deliberately not durable current-state authority because watchdog recovery can legitimately replace that generation; inspect live health/status when exact runtime identity is required. R1 owner isolation, R2 fail-closed lifecycle preflight, and R3 context-retention/continuation remain compatibility gates; MSSR 0.2.71 adds the portable first R4 semantic-consistency slice without claiming Bridge producer wiring for reserved semantic Context Messages.
