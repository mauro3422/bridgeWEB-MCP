...
## Current release
Bridge source/release candidate `0.6.139` now consumes exact `@mauroprime/mssr 0.2.71` from the verified vendored artifact (`c95a28aa4d5aac92eeda85a64d2254373c8178684aa6ae6484547df538e50af5`). The live runtime is still Bridge `0.6.138` with MSSR `0.2.70` on PID `8960` / boot `6050636a-a905-40eb-8c65-1985139fe7fb` until the `0.6.139` release commit is verified, published, and adopted through a controlled HTTP restart. R1 owner isolation, R2 fail-closed lifecycle preflight, and R3 context-retention/continuation remain compatibility gates; MSSR 0.2.71 adds the portable first R4 semantic-consistency slice without yet claiming Bridge producer wiring for reserved semantic Context Messages.
