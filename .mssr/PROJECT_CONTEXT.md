...
- Dashboard analytics execute in an isolated worker and return one bounded, cached snapshot. The HTTP/MCP event loop owns transport and liveness responsiveness and must not synchronously aggregate the observability stores used by the dashboard.
- Durable observability is liveness-isolated from the HTTP/MCP request path: Bridge enqueues MSSR/tool-metric persistence to one bounded single-writer worker, exposes pending evidence through a read-your-writes overlay for current projections, and runs passive WAL maintenance after quiet-period debounce. Portable MSSR semantics are unchanged by this host persistence strategy.
