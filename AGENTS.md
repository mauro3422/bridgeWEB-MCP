...
- `skill_bootstrap` usa ensamblado `selective` y planner global `global-required-core-first`: reserva todos los cores requeridos antes de módulos, carga módulos requeridos, ordena globalmente los opcionales y admite una skill opcional sólo si entra su paquete mínimo completo. Usa `contentMode=full` sólo para diagnóstico/rollback; `skill_load` continúa siendo la lectura completa explícita. Skills sin manifiesto caen a full con fallback observable. Sólo contexto requerido puede desbordar con evidencia; módulos ya cubiertos no se duplican. `exclusiveGroup` representa alternativas reales, no una optimización genérica: un empate devuelve candidatos sin inyectar ambas reglas.
