...
- `skill_bootstrap` usa ensamblado `selective` por defecto para skills propias con `context-modules.json`: carga el core y sólo módulos coincidentes con stage/intent dentro de `maxContextChars`. Usa `contentMode=full` sólo para diagnóstico/rollback; `skill_load` continúa siendo la lectura completa explícita. Skills sin manifiesto caen a full con fallback observable. Una skill opcional que no entra se omite completa; sólo una requerida puede desbordar con evidencia. `exclusiveGroup` representa alternativas: un empate devuelve candidatos sin inyectar ambas reglas.
