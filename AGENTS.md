...
- `skill_bootstrap` usa ensamblado `selective` y planner global `global-required-core-first`: reserva todos los cores requeridos antes de módulos. Para `caller=chatgpt-web`, los roots requeridos siguen siendo obligaciones y los roots opcionales se tratan como candidatas host-gated: sólo se materializan cuando existe una decisión `accepted`; su cierre transitivo de dependencias se carga junto con el root aceptado. Una dependencia que existe sólo por un root opcional skipped no se vuelve obligación independiente. `skipped` queda como feedback observable con motivo acotado y no cuenta como fallo de carga. Otros callers pueden conservar `auto` por compatibilidad. `contentMode=full` queda para diagnóstico/rollback; `skill_load` continúa siendo la lectura completa explícita.
