...
try {
  ...
  if (registry.tools.length !== 146) throw new Error(`expected 146 tools, got ${registry.tools.length}`);
  ...
  if (catalogComparison.runtime.count !== 146 || catalogComparison.mssr.runtime !== 12) throw new Error('connector catalog comparison runtime baseline failed');
  ...
  if (aliasAudit.summary?.registeredTools !== 146 || !aliasAudit.items?.some((item) => item.tool === 'work_once' && item.status === 'clarify')) throw new Error('live registry alias audit failed');
