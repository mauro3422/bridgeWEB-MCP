...
try {
  ...
  if (registry.tools.length !== 144) throw new Error(`expected 144 tools, got ${registry.tools.length}`);
  ...
  if (catalogComparison.runtime.count !== 144 || catalogComparison.mssr.runtime !== 12) throw new Error('connector catalog comparison runtime baseline failed');
  ...
  const expectedNeutral = ['whiteboard_capture_pc_view', 'whiteboard_add_text', 'whiteboard_add_svg', 'whiteboard_add_diagram', 'whiteboard_insert_image', 'mssr_trace_working_update', 'mssr_trace_record', 'mssr_observatory_epoch_start'];
  ...
  if (!mediaReviewTool.inputSchema?.properties?.files || !mediaReviewTool.inputSchema?.properties?.transcribe || !mediaReviewTool.inputSchema?.properties?.frameIntervalSeconds) throw new Error('Authorized media review schema failed');
  if (JSON.stringify(mediaReviewTool._meta?.['openai/fileParams']) !== JSON.stringify(['files'])) throw new Error('Authorized media review fileParams metadata failed');
  if (!mediaReviewFileDef?.properties?.download_url || !mediaReviewFileDef?.properties?.file_id || !mediaReviewFileDef?.properties?.mime_type || !mediaReviewFileDef?.properties?.file_name) throw new Error('Authorized media review file schema properties failed');
  if (JSON.stringify(mediaReviewFileDef.required) !== JSON.stringify(['download_url','file_id'])) throw new Error('Authorized media review file schema required fields failed');
  ...
  if (aliasAudit.summary?.registeredTools !== 144 || !aliasAudit.items?.some((item) => item.tool === 'work_once' && item.status === 'clarify')) throw new Error('live registry alias audit failed');
