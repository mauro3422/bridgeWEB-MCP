...
try {
  ...
  if (registry.tools.length !== 147) throw new Error(`expected 147 tools, got ${registry.tools.length}`);
  ...
  if (catalogComparison.runtime.count !== 147 || catalogComparison.mssr.runtime !== 12) throw new Error('connector catalog comparison runtime baseline failed');
  ...
  const expectedNeutral = ['mssr_context_ack','whiteboard_capture_pc_view', 'whiteboard_add_text', 'whiteboard_add_svg', 'whiteboard_add_diagram', 'whiteboard_insert_image', 'mssr_trace_working_update', 'mssr_trace_record', 'mssr_observatory_epoch_start'];
  ...
  if (aliasAudit.summary?.registeredTools !== 147 || !aliasAudit.items?.some((item) => item.tool === 'work_once' && item.status === 'clarify')) throw new Error('live registry alias audit failed');
