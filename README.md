...
```text
bridge-mcp source release v0.6.135
Live runtime: v0.6.134 (0.6.135 adoption pending controlled restart)
Packaged MSSR: 0.2.67
Mode: Streamable HTTP live; contextual notice delivery remains single-transport, with Document Freshness + explicit Situation semantic claims staged for 0.6.135 live adoption
...
Desde 0.6.105, C2e conecta proyecto+memoria con el mismo plano: `src/project-situation.ts` compara receipts operativamente activos del Context Plane contra revisiones canónicas actuales y entrega la evidencia a C2c/C2d. Desde 0.6.135 / MSSR 0.2.67, el mismo watcher compone además C2e-D explicit semantic claims sin exigir receipts: Bridge reutiliza `src/release-consistency.ts` para producir facts escalares de package/source/generated/installed/runtime y MSSR los normaliza/evalúa. Por defecto ese productor automático existe sólo para el propio contrato de release de Bridge; otros proyectos requieren un productor estructurado explícito y nunca se deriva verdad desde prosa libre. `/api/mssr/project-situation` sigue exponiendo sólo metadata acotada. `noticeClass`, categoría y prioridad enrutan atención sin crear otra cola ni autoridad; Bridge sólo muestra acciones C2d `ready`. Ver `docs/SITUATION_MODEL_ADAPTER.md`.
