...
## 2026-09-16 — Multi-chat `Connection failed`: pooled HTTP resets + false watchdog recovery

**Estado:** Corregido en source para Bridge 0.6.126; verificación funcional completa y estrés live pasaron. Adopción/publicación final se registra en el release 0.6.126.

**Capa / owner:** transporte HTTP/MCP Bridge ↔ `tunnel-client`, política de readiness del watchdog y hot paths filesystem/context de Bridge. MSSR conserva ownership semántico del routing; este incidente es host/runtime.

**Síntoma observable:** mientras un chat seguía usando Bridge, otros chats podían recibir `Connection failed` o 502 al inicializar una sesión. El runtime no siempre había caído: en una reproducción mantuvo exactamente el mismo PID/boot mientras el túnel incrementó `initialize 502` y el `POST /mcp` local sin status en la misma unidad.

**Evidencia mínima:** con logging debug del túnel (sin raw HTTP) se capturó `wsarecv: An existing connection was forcibly closed by the remote host` al reenviar `initialize` a `127.0.0.1:3001/mcp`. Los contadores acumulados de `tunnel_service_status=502` coincidieron exactamente con los POST locales sin status. El Bridge live anunciaba `Keep-Alive: timeout=5`. En una segunda clase de incidentes el watchdog escribió `auto-restart-http-readiness-threshold` aunque el proceso Node seguía vivo; la instrumentación observó stalls del event loop de varios segundos, con un máximo cercano a 16,7 s durante el diagnóstico.

**Causa demostrada:** había dos mecanismos independientes. (1) Node cerraba conexiones keep-alive a los 5 s mientras el cliente Go del túnel reutilizaba conexiones HTTP/1.1 pooled; una carrera al reutilizar un socket recién cerrado podía resetear un POST `initialize`. (2) el watchdog convertía tres probes de readiness fallidos en un kill aunque el proceso siguiera vivo, por lo que presión temporal del event loop podía transformarse en una caída real. Además, hot paths de filesystem/skill discovery repetían resolución canónica síncrona y `project_context_load(includeGuides=false)` seguía ejecutando recomendación de guides, amplificando presión evitable.

**Corrección:** HTTP usa keep-alive 120 s, buffer 5 s y headers timeout 130 s. `bridge_health`/`tunnel_health` correlacionan 502 con POST locales sin status y mantienen baseline por proceso del túnel. `/status` expone lag/stalls/client errors y lifecycle bounded. Navegación/escritura/discovery migraron los hot paths a path-policy async preparada sin relajar controles de symlink/denied roots. `includeGuides=false` omite recomendación de guides. El watchdog reinicia inmediatamente sólo si el child murió; para proceso vivo exige threshold de fallos **y** 90 s de unreadiness sostenida, capturando `/status` bounded antes de cualquier kill.
