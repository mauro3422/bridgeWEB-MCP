...
function bridgeServerOptions() {
  return {
    ...
    instructions: [
      ...
      "Use skill_route_plan for inspection. When applying a route, follow its nextAction and call skill_bootstrap. If bootstrap returns mustContinue=true, call its exact skill_context_next action repeatedly on the same trace until status=complete before dependent work; notices are recovery reminders, not context transport. A complete response exposes lifecycleGate: execute any exact postContextAction first, then finish the real phase and checkpoint/replan/close explicitly; context delivery never proves use or success. Oversized workflow guides are deliberately split into workflow_guide_load instead of consuming the compact bootstrap envelope. Required obligations and accepted optional roots remain distinct across every page. Use mssr_context_proposal_review before considering a delivered persistence proposal; review-ready is not write authorization. Use individual skill_load only for focused recovery. Before tools that consume runtime ids, follow metadata.usage preflights and never invent sessions, snapshots, uploads, Studios or provider tool names.",
