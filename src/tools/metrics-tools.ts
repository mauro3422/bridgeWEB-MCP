...
export const metricsToolModule: BridgeToolModule = {
  ...
  tools: [
    ...
    { name: "bridge_metrics_summary", description: "Return Bridge tool metrics segmented by surface and observable agent profile, including substantive MSSR routing coverage by chain. Diagnostics and route/bootstrap hooks stay observable but are excluded from the substantive denominator. Active scope starts at the current epoch; all preserves history.", inputSchema: { type: "object", properties: { scope: { type: "string", enum: metricScopes, default: "active" }, limit: { type: "number", default: 50, minimum: 1, maximum: 200 } }, additionalProperties: false } },
