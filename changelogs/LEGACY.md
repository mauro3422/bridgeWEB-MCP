...
- Add read-only `project_context_audit` governance across a workspace: classify Git repositories as modular, legacy, invalid, empty-bridge, not-initialized, or unmanaged; validate manifests with the canonical MSSR schema; and report migration debt without creating project memory.
- Make `project_context_load` surface missing-manifest governance warnings whenever a repository already has a `.bridge` authority directory. Existing `PROJECT_CONTEXT/PROJECT_MEMORY/PROJECT_STATE` files are reported as legacy authorities; an empty `.bridge` directory is reported as uninitialized authority instead of silently looking healthy.
- Keep project-memory mutation explicit: `project_context_update` remains the stable-section writer/manifest transaction, and neither audit nor load invents durable facts, state, or memory from telemetry or empty templates.
...
- Add explicit close preflight/notices with `closureDue`, missing gates and `nextRequiredAction`; idle remains a stale-open signal rather than inferred completion. Open traces older than the auto-recovery window remain explicitly resumable but no longer compete with fresh stateless routing.
- Fix persisted maintenance reconstruction so a valid close `phase_completed` checkpoint with omitted status updates `maintenanceRevision`, matching the portable MSSR reducer and preventing false stale-close blocks after restart.
- Add focused regressions for host-gated loading, contextual selection feedback, ephemeral working-memory purge, close preflight, stale auto-recovery and external `skill_decision` ingest.
...
- Make narrated video review event-adaptive by default: analyze reduced frames at a bounded cadence, correlate content changes and apparent 2D view motion with speech/audio metadata, persist only sparse coverage plus representative event JPEGs, emit `visualEvents`/motion-window references in timeline records, and write a Unicode `transcript.srt` sidecar when ASR returns text. Representative selection now prioritizes scene/content changes across the full recording instead of letting early camera motion consume the image budget.
- Improve narrated-media ASR with a hybrid canonical-text pipeline: recognize the full audio once for maximum linguistic context, retain bounded speech-aware recognitions as timestamp anchors, align the canonical transcript back onto those segments, preserve raw segment ASR for diagnostics, and use aligned canonical text in SRT/timeline/frame/event excerpts. Full-audio recognition is best-effort and degrades observably to the segmented transcript when unavailable.
- Fix `edit_lines` EOF handling: line tools now treat a terminal newline as file termination instead of a phantom blank logical line, preserve the original final-newline state and CRLF/LF style, ignore only the virtual trailing line from replacement content, and retain intentional blank lines. Add live dogfood/regression coverage for LF, CRLF, no-final-newline and intentional blanks.
